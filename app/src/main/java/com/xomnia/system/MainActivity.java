...
public class MainActivity extends Activity {
    ...
    // Отложенный запрос доступа к микрофону/камере от WebView, пока ждём
    // ответа пользователя на системный диалог (см. onPermissionRequest и
    // onRequestPermissionsResult) — без этого поля WebView либо получал бы
    // мгновенный отказ (если диалог ещё не отвечен), либо не имел бы
    // возможности вообще дождаться результата.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // в звонках X Forum). РАНЬШЕ здесь просто проверялось текущее
        // системное разрешение и, если его ещё не было, сразу звался
        // request.deny() — из-за этого звонок мог "не работать" даже после
        // того, как пользователь разрешал доступ в системном диалоге: сам
        // диалог запускался отдельно при старте приложения (см.
        // ensureMediaPermissions), и если WebView успевал спросить доступ
        // РАНЬШЕ, чем пользователь успевал ответить на этот диалог — отказ
        // уже случился, а второй попытки WebView сам не делает. Теперь,
        // если разрешения не хватает, запрос от WebView не отклоняется
        // сразу, а откладывается (pendingWebViewPermissionRequest) и
        // системный диалог запускается ПРЯМО В ЭТОТ МОМЕНТ — ответ
        // приходит в onRequestPermissionsResult ниже, где запрос наконец
        // подтверждается или отклоняется уже по факту настоящего ответа
        // пользователя.
            ...
            public void onPermissionRequest(final PermissionRequest request) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        java.util.List<String> neededManifestPerms = new java.util.ArrayList<String>();
                        for (String r : requestedResources) {
                            if (r.equals(PermissionRequest.RESOURCE_AUDIO_CAPTURE)
                                ...
                                neededManifestPerms.add(Manifest.permission.RECORD_AUDIO);
                            }
                            if (r.equals(PermissionRequest.RESOURCE_VIDEO_CAPTURE)
                                    && MainActivity.this.checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                                neededManifestPerms.add(Manifest.permission.CAMERA);
                            }
                        }
                        if (neededManifestPerms.isEmpty()) {
                            // Уже всё разрешено на уровне системы — подтверждаем сразу.
                            request.grant(requestedResources);
                            return;
                        }
                        pendingWebViewPermissionRequest = request;
                        requestPermissions(neededManifestPerms.toArray(new String[0]), MEDIA_PERMISSION_REQUEST_CODE);
                    }
                });
            }
        });
    ...
    // Настоящий ответ пользователя на системный диалог доступа к
    // микрофону/камере приходит сюда — ИМЕННО здесь (а не в момент самого
    // onPermissionRequest) нужно наконец подтвердить или отклонить
    // отложенный запрос WebView (pendingWebViewPermissionRequest), если он
    // есть. До этой правки WebView либо получал мгновенный отказ, либо
    // (при первом запросе от ensureMediaPermissions при старте приложения,
    // без связи с самим звонком) результат вообще никак не обрабатывался.
    @Override
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        final PermissionRequest request = pendingWebViewPermissionRequest;
    ...
    public class JsBridge {
        ...
        // ── Настоящее переключение раскладки клавиатуры (Shift+Alt) ─────────
        // В отличие от предыдущей попытки через атрибут lang на <input>
        // (ненадёжная подсказка, которую IME может игнорировать), это
        // РЕАЛЬНЫЙ системный API — InputMethodManager.switchToNextInputMethod
        // с onlyCurrentIme=true переключает именно ПОДТИП текущей клавиатуры
        // (например RU→EN внутри Gboard), а не саму клавиатуру целиком на
        // другое приложение. Именно так работает Shift+Alt/Win+Space в самой
        // Windows — здесь то же самое, но вызванное вручную из приложения,
        // а не самой ОС по нажатию клавиш (что для WebView в принципе
        // недоступно — см. предыдущий разговор).
        @JavascriptInterface
            ...
            activity.runOnUiThread(new Runnable() {
                public void run() {
                    try {
                        ...
                        // На части прошивок (особенно кастомных/старых OneUI)
                        // этот вызов может отсутствовать или падать — тогда
                        // просто ничего не происходит, без краша приложения.
                    }
                }
            });
        }
